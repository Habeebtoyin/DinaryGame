...
const NoMoves = ({ score, trackingScore }) => {
	...
	const GamePass = new GamePassNftContract(
		GAME_PASS_ADDRESS,
		signer,
		provider
	);
		...
		await GamePass.BuyMove()
			.then(async (res) => {
				await updateUserGameData(address.toString(), {
					...
					console.log(
						"nft bought , updated user moves that have been bought"
					);
				});
			...
			.catch((err) => {
				...
				toast.error(
					JSON.parse(JSON.stringify(err)).info.error.data.message
				);
			});
	};
	return (
			...
			<div>
				<div className="flex flex-col gap-2 justify-center items-center bg-white rounded-sm p-8 my-4">
					<Image
						...
						height={150}
					/>
					<p className="font-bold"> Dyleum NFT Game Pass</p>
				</div>
				...
				<div className="flex flex-col bg-white rounded-sm p-8">
					...
					<button
						onClick={() => buyGamePass()}
						className="bg-[#0045AD] text-center text-white px-5 py-2 rounded-[4px] my-4"
					>
						{isLoading === true ? "Buying More move" : "Buy Moves"}
					</button>
				</div>
			</div>
		</div>
