...
const AdminSendRewardForm = () => {
    const{transferNft, isConnected, isSuccessful, isLoading,setIsLoading,
		setSuccessful,
            ...
            <form action="" className="my-5 bg-white text-black max-lg:w-[100%] w-[40%] p-4 rounded-[5px]">
                ...
                <div>
                    <div className="mb-3">
                        ...
                        <input onChange={(e)=>setTo(e.target.value)} placeholder="Enter the recepient wallet address" type="text" className="border border-black w-[100%] p-3 rounded-[3px] outline-none" />
                    </div>

                    <div className="mb-3">
                        <label className="block mb-2 font-semibold text-[1.2rem]" >Reward Type</label>
                        <select name="" id="" value={selectedValue} onChange={handleSelectChange} placeholder="Select the Reward Type" className="border border-black w-[100%] p-3 rounded-[3px] outline-none">
                    ...
                    <div className="mb-3">
                        ...
                        <input onChange={(e)=>SetAmount(e.target.value)} placeholder="Enter the reward quantity" type="text" className="border border-black w-[100%] p-3 rounded-[3px] outline-none" />
                    </div>

                    <div className="my-6">
                        <input onClick={async ()=>{
                            let _nftType="master"
                            if(nftType==="legendary"){
                                _nftType="legendary"
                            }
                            await transferNft(amount,to,nftType)
                            if(error){
                                console.log({error})
                            }
                        }} type="button" value="Send" className="block w-[100%] text-white my-[1rem] bg-[#0045AD] px-5 py-3 rounded-[4px]"/>
                    </div>
                   {isLoading==true?toast.loading("Transferring NFt"):<></>}
                   {isSuccessful==true?toast.success('Nft transfer successfully'):<></>}
